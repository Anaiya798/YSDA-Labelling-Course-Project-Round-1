...
if __name__ == '__main__':
    ...
    data_path = '../dataset/classification/train.csv'
    ...
    dataset = CarNumbersDataset(data_path, transform=transform)
    ...
    model = MicroNet(cfg)
    module_state = torch.load('./model_versions/micronet-m3.pth')
    new_module_state = OrderedDict()
    for k, v in module_state.items():
        name = k[7:]
        new_module_state[name] = v

    model.load_state_dict(new_module_state)
